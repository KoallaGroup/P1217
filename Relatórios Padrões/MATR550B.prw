...
BeginSql Alias cAliasQry
	SELECT F2_DOC,F2_SERIE,F2_EMISSAO,F2_TIPO,F2_ICMSRET,F2_CLIENTE,F2_LOJA,
	       F2_FRETE,F2_FRETAUT,F2_ICMAUTO,F2_VALBRUT,F2_VALIPI,F2_VALICM,F2_VALISS,F2_FILIAL,F2_VEND1,F2_EST,
	       D2_DOC,D2_SERIE,D2_COD,D2_GRUPO,D2_TP,D2_TIPO,D2_CLIENTE,D2_LOJA,D2_GRADE,
	       D2_CF,D2_TES,D2_LOCAL,D2_PRCVEN,D2_ICMSRET,D2_QUANT,D2_TOTAL,D2_EMISSAO,D2_ALIQCMP,D2_PICM,
...
While !oReport:Cancel() .And. !(cAliasQry)->(Eof())
	...
	cfilSF2X	:= (cAliasQry)->F2_FILIAL
	cvend01X	:= (cAliasQry)->F2_VEND1
	cest01X		:= (cAliasQry)->F2_EST
	cali01X		:= (cAliasQry)->D2_ALIQCMP
	cpicm01X	:= (cAliasQry)->D2_PICM
	...
	While (cAliasQry)->(! Eof()) .AND. (cAliasQry)->D2_DOC == cNota .AND. (cAliasQry)->D2_SERIE == cSerie
		...
		If (cAliasQry)->D2_GRADE == "S" .And. MV_PAR09 == 1
			...
			Else
				SA7->(dBSetOrder(2))
				If SA7->(dBSeek(xFilial("SA7")+(cAliasQry)->D2_COD+(cAliasQry)->F2_CLIENTE+(cAliasQry)->F2_LOJA))
...
If SerieNfId("SF2",3,"F2_SERIE")<> "F2_SERIE"
	//campos do SF2
	cQuery:="SELECT F2_DOC,"+SerieNfId("SF2",3,"F2_SERIE")+ ",F2_SERIE,F2_EMISSAO,F2_TIPO,F2_ICMSRET"
	...
	//campos do SD2
	cQuery+=",D2_DOC,"+SerieNfId("SD2",3,"D2_SERIE")+",D2_SERIE,D2_COD,D2_GRUPO,D2_TP,D2_TIPO,D2_CLIENTE,D2_LOJA,D2_GRADE,D2_CF,D2_TES,D2_LOCAL,D2_ITEMPV,D2_PEDIDO"
Else
	//campos do SF2
	cQuery:="SELECT F2_DOC,F2_SERIE,F2_EMISSAO,F2_TIPO,F2_ICMSRET"
	cQuery+=",F2_FRETE,F2_FRETAUT,F2_ICMAUTO,F2_SEGURO,F2_DESPESA,F2_VALBRUT "
	cQuery+=",F2_VALIPI,F2_VALICM,F2_VALISS,F2_FILIAL,SF2.R_E_C_N_O_ SF2RECNO "
	//campos do SD2
...
cQuery+=" AND "+SerieNfId("SF2",3,"F2_SERIE")+" >='"+mv_par07+"' AND "+SerieNfId("SF2",3,"F2_SERIE")+"<='"+mv_par08+"'"
...
While !Eof() .and. lContinua
	...
	cSerieView	:= (cAliasSF2)->&(SerieNfId("SF2",3,"F2_SERIE"))
	...
	If (nAcN2+nAcN4+nAcN5) # 0
		...
		@Li ,   0 PSAY STR0014+cNota+" / "+cSerieView+" ---->"		//"TOTAL DA NOTA - "
...
While !Eof() .and. lContinua
	...
	While !Eof() .and. D2_DOC+D2_SERIE == cNota+cSerie
		...
		Else
			...
			lCompIPI := If((cAliasSD2)->D2_TIPO == "P",.T.,.F.)
...
cAliasSD2 := cAliasSF2
If SerieNfId("SF2",3,"F2_SERIE") <> "F2_SERIE"
	cQuery:="SELECT F2_CLIENTE,F2_LOJA,F2_DOC,"+SerieNfId("SF2",3,"F2_SERIE")+", F2_SERIE,F2_EMISSAO"
Else
...
If SerieNfId("SF1",3,"F1_SERIE") <> "F1_SERIE"
	cQuery:="SELECT F1_FORNECE,F1_LOJA,F1_DOC,"+SerieNfId("SF1",3,"F1_SERIE")+",F1_SERIE,F1_DTDIGIT"
Else
	cQuery:="SELECT F1_FORNECE,F1_LOJA,F1_DOC,"+SerieNfId("SF1",3,"F1_SERIE")+",F1_SERIE,F1_DTDIGIT"
Endif
...
cQuery+=" AND "+SerieNfId("SF1",3,"F1_SERIE")+">='"+mv_par07+"' AND "+SerieNfId("SF1",3,"F1_SERIE")+"<='"+mv_par08+"'"
...
While (!(cAliasSF1)->(Eof()) .Or. !(cAliasSF2)->(Eof()) ).And. lContinua
	...
	ElseIf  !(cAliasSF2)->(Eof())
		...
		cNota		:= (cAliasSF2)->F2_DOC
		cSerie     	:= (cAliasSF2)->&(SerieNfId("SF2",3,"F2_SERIE"))
