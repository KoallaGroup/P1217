...
User Function LOJRNFCe(    oNFCe        , oProt        , nDecimais    , aFormas    ,;
    ...
    oPrint := FWMsPrinter():New("Impressão NFC-e", IMP_PDF, lAdjustToLegacy,cPath)
...
User Function LjRImpNFCE(    oNFCe        , oProt        , nDecimais    , aFormas    ,; 
    ...
    */
    ...
    For nX := 1 to Len(aItemNfce)
        ...
        oPrint:Say( nAuxLn,aColDet[02, 02],PADR(cDescrPro,nTamCol)    + space(1)  ,oFont1 )                    //Descricao de Produto
        ...
        cDescrPro := Substr(cDescrPro,  nTamCol+1)
        Do While !Empty(cDescrPro)
            ...
            EndIf
            oPrint:Say( nAuxLn,aColDet[02, 02],PADR(cDescrPro, nTamCol )    + space(1)  ,oFont1 )                    //Descricao de Produto
            cDescrPro := Substr(cDescrPro, nTamCol)
        EndDo
        If nAuxLn > nTamPgV
            nAuxLn := 20
            oPrint:EndPage()
...
Local lSTImPFNfce    := FindFunction("STImPFNfce") .And. STImPFNfce()
Local cImgQrCode    := ""
Local aMensagem        := ""
...
BEGIN SEQUENCE
    ...
    If (lDanfePad .AND. lLJ7084) .Or. lSTImPFNfce
        ...
        If !lImpComum .AND. !lPos .AND. nHdlECF == -1        
            ...
            If nHdlECF == -1
                If !IsBlind()
                    If FindFunction("LjObgCpVen") .And.    FindFunction("INFCon")    //LOJNFCE.PRW e LOJXECF.PRX
