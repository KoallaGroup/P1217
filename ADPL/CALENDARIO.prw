...
Function U_Calendario()
    ...
    oCalend:SetbNewActivity( {|dDate,cTimeIni,cTimeFin| U_NewTask(dDate,cTimeIni,cTimeFin) } )
    oCalend:SetbClickActivity( {|oItem| U_EditTask(oItem) } )
    oCalend:SetbRefresh( {|dDate| U_BuscaAtividades(dDate) } )
    oCalend:SetbRightClick( {|oItem| U_RightClick(oItem) } )
...
Function U_NewTask(dDate, cTimeIni, cTimeFin)
...
Function U_EditTask(oItem)
    Local aArea     := GetArea()
    DbSelectArea('ZZL')
    DbSetOrder(1)
    If DbSeek(xFilial('ZZL') + oItem:cID)
        FWExecView("",'U_MVC_CALEND', MODEL_OPERATION_UPDATE,, { || .T. } , , 50 )
    EndIf
    RestArea(aArea)
Return .T.
Function U_ExcluirAtiv(cID)
    ...
    If DbSeek(xFilial('ZZL') + cID)
        FWExecView("",'U_MVC_CALEND', MODEL_OPERATION_DELETE,, { || .T. } , , 50 )
    EndIf
    RestArea(aArea)
Return
//Funcao chamada ao clicar com o botao direito
Function U_RightClick(oItem)
    Local aMenu := {}
    ...
    //-------------------------------------------------------
        AADD(aMenu, {'Excluir', "U_ExcluirAtiv('" + oItem:cID + "')" } )
    Else
    //-------------------------------------------------------
...
Function U_BuscaAtividades(dDate)
    Local aArea     := GetArea()
    Local aItems    := {}
    Local oItem     := nil
